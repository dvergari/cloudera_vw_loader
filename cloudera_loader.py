...
class ClouderaVWLoader(BaseLoader):
    """Load from `Cloudera Virtual Warehouse`.
    ...
    def __init__(
        ...
        """Initialize Cloudera document loader.

        Args:
            query: The query to run in Cloudera CDW.
            workload_user: Cloudera workload user.
            workload_password: Cloudera workload password.
            endpoint: Cloudera VW enpoint.
            database: Cloudera VW database
            parameters: Optional. Parameters to pass to the connection.
    ...
    def _execute_query(self) -> List[Dict[str, Any]]:
        try:
            ...
            raise ImportError(
                "Could not import impyla package. "
                "Please install it with `pip install impyla`."
            ) from ex
        conn = connect(host=self.endpoint,
                       port=443,
